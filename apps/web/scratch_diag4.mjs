...
async function run() {
  ...
  // We must use the full Voice Name, not just the short name!
  // MsEdgeTTS internally maps short names to full names, but we need to do it ourselves if we bypass it.
  ...
  stream.audioStream.on("end", () => {
    console.log("DONE. Audio bytes:", audioBytes);
  });
