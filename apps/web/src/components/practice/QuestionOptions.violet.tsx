          ...
          <button
            ...
            className={`w-full relative group rounded-2xl p-5 sm:p-6 text-left transition-all duration-200 focus:outline-none focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-white forced-colors:focus-visible:outline-[Highlight] ${
              isSelected && selectedAnswer?.is_correct === true
                ? "border-[3px] border-b-[6px] border-emerald-500 bg-emerald-950/40 text-emerald-50 shadow-[4px_4px_0px_0px_rgba(16,185,129,1)]"
                : isSelected && selectedAnswer?.is_correct === false
                ? "border-[3px] border-b-[6px] border-red-500 bg-red-950/40 text-red-50 shadow-[4px_4px_0px_0px_rgba(239,68,68,1)]"
                : isCorrectOption && selectedAnswer
                ? "border-[3px] border-b-[6px] border-emerald-500 bg-emerald-950/40 text-emerald-50 shadow-[4px_4px_0px_0px_rgba(16,185,129,1)]"
                : isSelected
                ? "border-[3px] border-b-[6px] border-white bg-white/10 text-white shadow-[4px_4px_0px_0px_rgba(255,255,255,1)]"
                : "border-[3px] border-b-[6px] active:translate-y-[3px] active:border-b-[3px] border-[#3e1272] hover:border-violet-300 hover:bg-[#3e1272]/50 text-violet-200 hover:text-white"
          ...
          >
            <div className="flex items-start gap-4 sm:gap-6">
              ...
              <div aria-hidden="true" className={`shrink-0 flex items-center justify-center w-12 h-12 rounded-xl border-[3px] text-xl font-black transition-colors ${
                isSelected && selectedAnswer?.is_correct === true ? "bg-emerald-500 border-emerald-500 text-[#120224]" :
                isSelected && selectedAnswer?.is_correct === false ? "bg-red-500 border-red-500 text-white" :
                isCorrectOption && selectedAnswer ? "bg-emerald-500 border-emerald-500 text-[#120224]" :
                isSelected ? "bg-white border-white text-[#120224]" :
                "bg-[#120224] border-[#3e1272] text-violet-300 group-hover:border-violet-300 group-hover:text-white"
              }`}>
                {option.value}
              </div>
              ...
              <div className="space-y-3 flex-1 pt-1">
                <p id={optionLabelId} className={`text-lg sm:text-xl md:text-2xl font-black leading-snug transition-colors ${
                  isSelected || (isCorrectOption && selectedAnswer) ? "text-white" : "text-violet-200"
                }`}>
