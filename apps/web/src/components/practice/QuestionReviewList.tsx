...
export function QuestionReviewList({ questions, initialBookmarkedIds = [] }: QuestionReviewListProps) {
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-5 shadow-[0_18px_50px_rgba(15,23,42,0.08)] sm:flex-row sm:items-center sm:justify-between sm:p-6">
        <div>
      ...
      <div className="space-y-4">
        {filteredQuestions.length === 0 ? (
          <div className="rounded-3xl border border-border bg-card px-6 py-10 text-center text-slate-600 shadow-sm dark:text-slate-300">
          ...
          filteredQuestions.map((question, index) => {
            ...
            return (
              <article
                key={question.id}
                className="overflow-hidden rounded-3xl border border-border bg-card shadow-sm transition"
              >
