...
import { usePracticeSession, useSubmitAnswer, useFinishAttempt, QuestionCard, SessionProgress, SessionSkeleton, QuestionNavigatorDrawer, ConfirmSubmitModal } from '../../../src/features/practice-engine'
...
export default function PracticeSessionScreen() {
  ...
  // Fallback: Manually announce the question since programmatic focus shifting is unreliable on some Android versions.
  useEffect(() => {
    const q = data?.questions?.[currentIndex]
    if (q) {
      setTimeout(() => {
        AccessibilityInfo.announceForAccessibility(toLatinNumerals(`Question ${currentIndex + 1}. ${q.content}`))
      }, 150)
    }
  }, [currentIndex, data?.questions])
  ...
  }, [data, currentIndex, submitAnswer])
    ...
    <View className="flex-1 bg-background">
        ...
        {currentQuestion.explanation && !!currentSelected ? (
          <View className="px-4 pb-6">
            <Button
              ...
              leftIcon={<Feather name={showExplanation ? "eye-off" : "eye"} size={20} className="text-primary" />}
              title={showExplanation ? "Hide Explanation" : "View Explanation"}
            />
            
            {showExplanation && (
              <View className="mt-3 rounded-xl bg-card p-4 border border-border">
      ...
      <View className="border-t border-border bg-card px-4 pb-8 pt-4">
        <View className="flex-row gap-3">
          <Button
            onPress={() => { setShowExplanation(false); currentIndex > 0 && setCurrentIndex((i) => i - 1) }}
            disabled={currentIndex === 0}
            accessibilityLabel="Previous question"
            variant="secondary"
            className="flex-1"
            leftIcon={<Feather name="chevron-left" size={20} className={currentIndex === 0 ? 'text-muted-foreground' : 'text-secondary-foreground'} />}
            title="Previous"
          />
          ...
          {isLastQuestion && canSubmit ? (
            ...
            <Button
              onPress={() => { setShowExplanation(false); currentIndex < questions.length - 1 && setCurrentIndex((i) => i + 1) }}
        ...
        <Button
          ...
          variant="heavyOutline"
          className="mt-3 w-full"
          leftIcon={<Feather name="grid" size={18} className="text-foreground" />}
      ...
      <QuestionNavigatorDrawer
        ...
        onSelect={(idx) => { setShowExplanation(false); setCurrentIndex(idx) }}
      />
